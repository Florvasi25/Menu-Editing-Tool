...
#sends the sections available to the mainwindow
    def slot_mostrarSecciones(self,datos,isAdmin):
        secciones = list()
        if(isAdmin == True):
            ...
            for section in datos['categories']:
                secciones.append(section['caption'])
...
#shows a list of the items on the menu and in which sections you can find them
    def slot_mostrarItems(self,datos,isAdmin):
        items = list()
        if (isAdmin == True):
            for section in datos['MenuSections']:
                ...
                for menuItem in section["MenuItems"]:
                    ...
                    menuItemName = section['Name'].upper() + ": " + menuItem["Name"]
                    items.append(menuItemName)
        else:
            for category in datos['categories']:
                for item in category["items"]:
                    menuItemName = category['caption'].upper() + ": " + item["caption"]
                    items.append(menuItemName)

        return items

#checks if any item inside an OS is empty
    def find_empty_valuess(self,datos):
                ...
                for optionSets in items["MenuItemOptionSets"]:
                    for optionSetItems in optionSets["MenuItemOptionSetItems"]:
                        if optionSetItems["Name"] == "" or optionSetItems["Name"] == None:
                            ...
                            emptyOSItems.append(seccion)
                            emptyOSItems.append(item)
                            emptyOSItems.append(optionSetTitle)

        return emptyOSItems

#find items with empty titles
    def get_item_names(self, datos):
        itemTitles = list()
        section = ""
        for sections in datos["MenuSections"]:
            for items in sections["MenuItems"]:
                if items["Name"] == "" or items["Name"] == None:
                    section = sections["Name"]
                    itemTitles.append(section)

        return itemTitles
    
#find empty OS Titles
    def slot_get_OS_titles(self,datos):
        optionSet = list()
        section = ""
        item = ""
        for sections in datos["MenuSections"]:
            for items in sections["MenuItems"]:
                for OS in items["MenuItemOptionSets"]:
                    if OS["Name"] == "" or OS["Name"] == None:
                        section = sections["Name"]
                        item = items["Name"]
                        optionSet.append(section)
                        optionSet.append(item)
